...
class ChunkedBackupDriver(driver.BackupDriver):
    ...
    def _get_object_writer(self, container, object_name, extra_metadata=None):
        ...
        LOG.info('_get_object_writer, container name: %(container)s, object name: %(object_name)s.',
                 {'container': container, 'object_name': object_name})
        LOG.info('_get_object_writer, extra_metadata: %(extra_metadata)s, writer: %(writer)s.',
                 {'extra_metadata': extra_metadata, 'writer': writer})
    ...
    def write_backup(self, data, container, object_name, extra_metadata):
        with self._get_object_writer(container, object_name, extra_metadata=extra_metadata) as writer:
            writer.write(data)

    def generate_object_data_for_object_meta(self, data, data_offset, object_name):
    ...
    def save_object_data_into_object_meta(self, obj):
        self.object_meta['list'].append(obj)
        self.object_meta['id'] += 1
    ...
    def _finalize_backup(self, backup, container, object_sha256):
        """Write the backup's metadata to the backup repository."""
        object_list = self.object_meta['list']
        object_id = self.object_meta['id']
        volume_meta = self.object_meta['volume_meta']
        sha256_list = object_sha256['sha256s']
        extra_metadata = self.object_meta.get('extra_metadata')
    ...
    def _backup_metadata(self, backup):
        ...
        self.object_meta["volume_meta"] = json_meta

    def _send_progress_end(self, context, backup):
        self.object_meta['backup_percent'] = 100
    ...
    def _send_progress_notification(self, context, backup,
                                    total_block_sent_num, total_volume_size):
        backup_percent = total_block_sent_num * 100 / total_volume_size
        self.object_meta['backup_percent'] = backup_percent
        volume_utils.notify_about_backup_usage(context,
                                               backup,
                                               "createprogress",
                                               extra_usage_info=
    ...
    def read_volume(self, backup, win32_disk_size, volume_file):
        ...
        if sys.platform == 'win32':
            read_bytes = min(self.chunk_size_bytes, win32_disk_size - data_offset)
        else:
            read_bytes = self.chunk_size_bytes
        LOG.info('read_bytes %s', read_bytes)
    ...
    def create_thread(self, object_id, backup, win32_disk_size, volume_file, sha256_list, container, extra_metadata):
        ...
        data, data_offset = self.read_volume(backup, win32_disk_size, volume_file)
        if data == b'':
            return None, None
    ...
    def backup(self, backup, volume_file, backup_metadata=True):
        ...
        (self.object_meta, object_sha256, extra_metadata, container,
         volume_size_bytes) = self._prepare_backup(backup)
        LOG.info("!!!")
        LOG.info('object_meta %s', self.object_meta)
        LOG.info('object_sha256 %s', object_sha256)
        LOG.info('extra_metadata %s', extra_metadata)
        LOG.info('container %s', container)
        LOG.info('volume_size_bytes %s', volume_size_bytes)
        ...
        while threads:
            ...
            self.save_object_data_into_object_meta(threads[0]['object'])
        ...
        writer_close_time = datetime.now()
        LOG.info('writer_close_time %s', (datetime.now() - writer_close_time).total_seconds())
        LOG.info('time_volume_file %s', time_volume_file)
        LOG.info('sum_backup_chunk_ %s', sum_backup_chunk_)
        LOG.info('Time2 %s', (datetime.now() - start_time2).total_seconds())
        # Stop the timer.
        timer.stop()
        ...
        self._send_progress_end(self.context, backup)
        ...
        if backup_metadata:
            ...
            # Whatever goes wrong, we want to log, cleanup, and re-raise.
                ...
                with excutils.save_and_reraise_exception():
                    ...
                    self.delete_backup(backup)
        LOG.info('Time %s', (datetime.now() - start_time).total_seconds())
        start_time = datetime.now()
        self._finalize_backup(backup, container, object_sha256)
        LOG.info('_finalize_backupTime %s', (datetime.now() - start_time).total_seconds())
        self.object_meta = None
